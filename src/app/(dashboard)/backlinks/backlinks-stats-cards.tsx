...
export function BacklinksStatsCards({ stats }: Props) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      <StatCard label="Backlinks" value={stats.totalBacklinks} icon={<Link2 className="w-4 h-4 text-accent" />} color="text-accent" />
      <StatCard label="Domains" value={stats.uniqueDomains} icon={<Globe className="w-4 h-4 text-sky-400" />} color="text-sky-400" />
      <StatCard label="Keywords" value={stats.uniqueKeywords} icon={<Hash className="w-4 h-4 text-purple-400" />} color="text-purple-400" />
