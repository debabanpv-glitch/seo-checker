  ...
  if (surging.length === 0 && dropping.length === 0) {
    return (
      <div className="bg-card border border-border rounded-xl p-8 text-center">
        <p className="text-[#8888a0] text-sm">Không có từ khóa biến động lớn (thay đổi &ge; 5 bậc)</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      <div className="bg-card border border-emerald-500/20 rounded-xl p-3">
        <div className="flex items-center gap-2 mb-2 px-2">
          ...
          <span className="text-sm font-semibold text-emerald-400">Tăng mạnh</span>
          <span className="text-[10px] text-[#8888a0]">({surging.length})</span>
        </div>
        {surging.length === 0 ? (
          <p className="text-[#666680] text-xs px-2 py-4 text-center">Không có</p>
        ) : (
          surging.map((kw) => (
      ...
      <div className="bg-card border border-red-500/20 rounded-xl p-3">
        <div className="flex items-center gap-2 mb-2 px-2">
          ...
          <span className="text-sm font-semibold text-red-400">Giảm mạnh</span>
